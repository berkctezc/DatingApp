...
## 9 - Updating Resources (FE+BE)

### Member edit component (FE)
...
### Adding the update functionality (FE)
...
### Updating user in the client app (FE)
...
### Using service to store state (FE)
...
## 10 - Photo Upload Functionality (BE+FE)
...
### Creating a reusable text input (FE)

- Created a **\_forms** folder and generate a component named **text-input** `ng g c text-input --skip-tests`
- Implement **ControlValueAccessor** in **text-input.component** instead of **OnInit**
...
### Using the form builder service (FE)
...
### Expanding the register form (FE)
...
### Adding Reusable date input (FE)

- Add **datepicker** module in shared.module.ts
...
## 12 - Paging, Sorting, Filtering (BE+FE)
...
### Adding helper classes for pagination (BE)
...
### Using the pagination classes (BE)
...
### Using the angular bootstrap pagination component (FE)
...
### Adding Additional Parameters (BE)
...
### Cleaning up the member service (BE+FE)
...
### Adding an action filter (LogUserActivity) (BE)
...
### Making the LastActive action filter more optimal (BE)
...
### Restoring the caching for members (FE)
...
### Restoring caching for member detailed (FE)
...
### Remembering the filters for a user in the service (FE)
...
- Create confirmation service in _services
...
- Create **confirm-dialog component** in **modals**
...
- **Prevent unsaved changes guard** update
...
- **messages.component** delete method update (Inject **ConfirmService**)
...
- **member-message** component change changeDetection method
...
- Searched for usages of localhost and change them with **environment.apiUrl**
- **angular.json** change output path to `"outputPath": "../API/wwwroot",`
- **Startup.cs** add `app.UseDefaultFiles(); app.UseStaticFiles();` to **Configure**
- run `ng build` for client
- now our app runs without angular and just from dotnet server

### Adding a fallback controller 

- Create **FallbackController** in Controllers
...
- Add  `endpoints.MapFallbackToController("Index", "Fallback");` to endpoints

### Creating an angular production build

https://angular.io/guide/deployment#production-optimizations

- Removed fake delay from **loading.interceptor.ts**
- `ng build --prod`
...
- Install Docker Desktop, find postgres on dockerhub
- run on terminal
...
- Check the instances from Docker Desktop app
- Install pgAdmin and login **Host**: localhost **Port**: 5432 **Maintenance database**: postgres **Username**: appuser **Password**: password

### Changing the DB Server in our app

- Delete migrations, drop db with `dotnet ef database drop`
- Install **Npgsql.EntityFrameworkCore.PostgreSQL** Nuget Package
- Change **ConnectionStrings** in **appsettings.Development.json**

```json
"ConnectionStrings": {
    "DefaultConnection": "Server=localhost;Port=5432;User Id=appuser;Password=password; Database=datingapp"
},
```

- Change **AddDbContext** in **ApplicationServiceExtensions**
...
- dotnet run to apply migrations
...
- Find heroku dotnet buildpack element
...
#login to heroku
heroku login
heroku git:remote -a angularnet5datingapp #appname on heroku
# set buildpack as dotnet core
heroku buildpacks:set jincod/dotnetcore
```

- Go to resources tab of the app on heroku
- add Heroku Postgres in Add-ons (Hobby Dev - Free)
-  Add config vars in settings app (CloudinarySettings:CloudName, ApiKey, ApiSecret...)

### Deploying the app to Heroku

- Use this as DataContext options in **ApplicationServiceExtensions** 
...
- set environment variable in terminal for heroku `heroku config:set ASPNETCORE_ENVIRONMENT=Production` and `heroku config:set TokenKey=A7Da2xHP4QzbxYzf6u2njqj7mjQ6jEHe`
- git push heroku master

### Using branches in git and connecting Heroku to GitHub

- Change deploy from Heroku Git to GitHub (dont forget to ng build to get new wwwroot files)
- Create a new branch for development git checkout -b FixMessages
- after changes `git add . ; git commit -m "commitMessage"; git push origin FixMessages`

### Merging changes and redeploying

- git checkout master;git merge FixMessages; git push
